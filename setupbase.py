"""
This module defines the things that are used in setup.py for building the nbclassic
...
loose_pep440re = re.compile(r'^([1-9]\d*!)?(0|[1-9]\d*)(\.(0|[1-9]\d*))*((a|b|rc)(0|[1-9]\d*))?(\.post(0|[1-9]\d*))?(\.dev(0|[1-9]\d*)?)?(\+juno)?$')
...
static = pjoin(repo_root, 'nbclassic', 'static')
        ...
        for p in paths:
            ...
            for component in ['nbclassic', 'nbui']:
                run(['pybabel', 'compile',
                     ...
                     '-f',
                     '-l', LANG,
                     '-i', pjoin('nbclassic', 'i18n', LANG, 'LC_MESSAGES', component+'.po'),
                     '-o', pjoin('nbclassic', 'i18n', LANG, 'LC_MESSAGES', component+'.mo')
                    ])
...
class CompileJS(Command):
    ...
    def build_jstranslation(self, trd):
        ...
        run([
            ...
            '-f', 'jed1.x',
            '-d', 'nbjs',
            pjoin('nbclassic', 'i18n', lang, 'LC_MESSAGES', 'nbjs.po'),
            pjoin('nbclassic', 'i18n', lang, 'LC_MESSAGES', 'nbjs.json'),
        ])
...
class JavascriptVersion(Command):
    """write the javascript version to nbclassic javascript"""
    ...
    def run(self):
        nsfile = pjoin(repo_root, "nbclassic", "static", "base", "js", "namespace.js")
